...
int mangle(int sid){
    sid = sid >> 2; // shifts the ID to the right by 2
    sid = sid & (~(1 << 6)); // clears bit 6
    sid ^= 1 << 3; // complements bit 3
    return sid;
}
...
int  bit_check(int data, int bits_on, int bits_off){
    ...
    for(int i = 0; i < sizeof(data) * 4; i++) {
        int on_bit = (bits_on >> i) & 1; // takes the ith bit from the on_bit
        int off_bit = (bits_off >> i) & 1; // takes the ith bit from the off_bit
        int data_bit = (data >> i) & 1; // takes the ith bit from the data_bit
        if(on_bit == 1 && off_bit == 1) {
            return -1;
        }
        else {
            if(on_bit == 1 && data_bit == 0) {
                answer = 0;
            }
            if(off_bit == 1 && data_bit == 1) {
                answer = 0;
            }
        }
    }
...
int ldif(char c1, char c2){
    if((strchr(a_array, c1) == '\0') || (strchr(a_array, c2) == '\0')) {
        return -69;
    }
    char *s;
    s = pmatch(c1);
...
void per_print(Person *p){
    ...
    printf("\n");
    printf("%-17s %s", "First Name:", (*p).FirstName);
    printf("\n");
    printf("%-17s %s", "Last Name:", (*p).LastName);
    printf("\n");
    printf("%-17s %0.60s", "Address:", (*p).StreetAddr);
    printf("\n");
    printf("%-17s %s", "Zip:", (*p).ZipCode);
    printf("\n");
    printf("%-17s %0.2f", "Height:", (*p).Height);
    printf("\n");
    printf("%-17s %0.2f", "Weight:", (*p).Weight);
    printf("\n");
    printf("%-17s %ld", "DOB 1/1/1900:", (*p).DBirth);
    printf("\n");
    printf("---   ---   --- \n");
}
